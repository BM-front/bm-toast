...
(async () => {
	...
	let newVersion =
		bumpType === 'custom'
			? (await inquirer.prompt([{ type: 'input', name: 'v', message: 'Введите новую версию:', default: defaultPatch }]))
					.v
		...
		const { cont } = await inquirer.prompt([
			{
				type: 'confirm',
				name: 'cont',
				message: 'В рабочем дереве есть изменения. Продолжить?',
				default: false,
			},
		]);
	...
	const { publish } = await inquirer.prompt([
		{
			type: 'confirm',
			name: 'publish',
			message: 'Опубликовать на npm сейчас?',
			default: true,
		},
	]);
